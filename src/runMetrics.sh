...
function saveTraces {		
    cat temp | awk -v var=$i '{print var"," $0}' | grep $gpu >> ../../../metrics/${app}-metrics.csv    
}
        ...
        for i in `seq 100000 100000 1000000`; do
            for j in `seq 10 10 100`; do
                for k in 2 4 8 16 32 64; do
					...
					cat temp | awk -v var=$i  -v var2=$j -v var3=$k '{print var"," var2"," var3"," $0}'  | grep $gpu >> ../../../metrics/${app}-metrics.csv									
                done
            done
        done
    fi
