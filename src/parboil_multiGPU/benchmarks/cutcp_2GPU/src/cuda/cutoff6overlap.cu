    ...
    )
{
  ...
  ener_t *regionZeroCuda1,*regionZeroCuda2;
  ...
  float4 *binBaseCuda1, *binZeroCuda1;
  float4 *binBaseCuda2, *binZeroCuda2;
  ...
  cudaStream_t cutoffstream1;
  cudaStream_t cutoffstream2;
cudaSetDevice(0);
  cudaStreamCreate(&cutoffstream1);
  cudaMalloc((void **) &regionZeroCuda1, lnall * sizeof(ener_t));
  CUERR;
  cudaMemset(regionZeroCuda1, 0, lnall * sizeof(ener_t));
  CUERR;
  if (verbose) {
    printf("Allocating %.2fMB on CUDA device for atom bins\n",
           nbins * BIN_DEPTH * sizeof(float4) / (double) (1024*1024));
  }
  cudaMalloc((void **) &binBaseCuda1, nbins * BIN_DEPTH * sizeof(float4));
  CUERR;
  cudaMemcpy(binBaseCuda1, binBaseAddr, nbins * BIN_DEPTH * sizeof(float4),
      cudaMemcpyHostToDevice);
  CUERR;
  binZeroCuda1 = binBaseCuda1 + ((c * binDim.y + c) * binDim.x + c) * BIN_DEPTH;
...
cudaSetDevice(1);
  cudaStreamCreate(&cutoffstream2);
  cudaMalloc((void **) &regionZeroCuda2, lnall * sizeof(ener_t));
  CUERR;
  cudaMemset(regionZeroCuda2, 0, lnall * sizeof(ener_t));
  CUERR;
  if (verbose) {
    printf("Allocating %.2fMB on CUDA device for atom bins\n",
           nbins * BIN_DEPTH * sizeof(float4) / (double) (1024*1024));
  }
  cudaMalloc((void **) &binBaseCuda2, nbins * BIN_DEPTH * sizeof(float4));
  CUERR;
  cudaMemcpy(binBaseCuda2, binBaseAddr, nbins * BIN_DEPTH * sizeof(float4),
      cudaMemcpyHostToDevice);
  CUERR;
  binZeroCuda2 = binBaseCuda2 + ((c * binDim.y + c) * binDim.x + c) * BIN_DEPTH;
  cudaMemcpyToSymbol(NbrListLen, &nbrlistlen, sizeof(int), 0);
  CUERR;
  cudaMemcpyToSymbol(NbrList, nbrlist, nbrlistlen * sizeof(int3), 0);
  CUERR;
...
cudaSetDevice(0);
  ...
  for (zRegionIndex = 0;  zRegionIndex < zRegionDim/2;  zRegionIndex++) {
    printf("  computing plane %d\r", zRegionIndex);
    fflush(stdout);
    cuda_cutoff_potential_lattice6overlap<<<gridDim, blockDim, 0,cutoffstream1>>>(binDim.x, binDim.y,
        binZeroCuda1, h, cutoff2, inv_cutoff2, regionZeroCuda1, zRegionIndex);
  }
  CUERR;
cudaSetDevice(1);
  for (zRegionIndex = zRegionDim/2;  zRegionIndex < zRegionDim;  zRegionIndex++) {
    fflush(stdout);
    cuda_cutoff_potential_lattice6overlap<<<gridDim, blockDim, 0,cutoffstream2>>>(binDim.x, binDim.y,
        binZeroCuda2, h, cutoff2, inv_cutoff2, regionZeroCuda2, zRegionIndex);
  }
  ...
  CUERR;
  cudaStreamSynchronize(cutoffstream2);
  CUERR;
cudaSetDevice(0);
  cudaStreamSynchronize(cutoffstream1);
  CUERR;


  printf("Finished CUDA kernel calls                        \n");
long offst;
offst=(lnall/zRegionDim) *(zRegionDim/2);
printf("offset =%d lnall=%d \n",offst,lnall );
  /* copy result regions from CUDA device */
  pb_SwitchToTimer(timers, pb_TimerID_COPY);
cudaSetDevice(0);
  cudaMemcpy(regionZeroAddr, regionZeroCuda1, offst * sizeof(ener_t),
      cudaMemcpyDeviceToHost);
  CUERR;
cudaSetDevice(1);
  cudaMemcpy(regionZeroAddr+offst, regionZeroCuda2+offst, (lnall-offst) * (sizeof(ener_t)),
      cudaMemcpyDeviceToHost);
  CUERR;
//for (long i=0;i<lnall;i++){
//printf("regionzeroAddr[%d]=%f\n",i,regionZeroAddr[i]);
//if (regionZeroAddr[i]==0.0  && i>309760)exit(-1);
//}
  /* free CUDA memory allocations */
cudaSetDevice(0);
  cudaFree(regionZeroCuda1);
  cudaFree(binBaseCuda1);
  cudaStreamDestroy(cutoffstream1);
cudaSetDevice(1);
  cudaFree(regionZeroCuda2);
  cudaFree(binBaseCuda2);
  cudaStreamDestroy(cutoffstream2);
