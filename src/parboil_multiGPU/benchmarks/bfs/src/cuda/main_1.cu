...
/*				BFS_kernel1<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes, 
				...
				if (num_of_blocks%2==0){
				 ...
				 dup_q2=d_q2;
					...
					printf("numt2=%d  num_t1=%d ",num_t2,num_t1);
				...
				(cudaMemcpy(&num_t1,tail1,sizeof(int),cudaMemcpyDeviceToHost));
					pthread_barrier_wait(&bar);
                                //(cudaMemcpy(&num_t2,tail2,sizeof(int),cudaMemcpyDeviceToHost));
                                      dim3  grid1( (int)ceil(num_t2/(double)MAX_THREADS_PER_BLOCK), 1, 1);
					printf("numt1=%d  %d ",num_t2,grid1.x);
                                  //      dim3  threads1(MAX_THREADS_PER_BLOCK, 1, 1);
                                        duplicate_detection<<<grid1,threads>>>(num_t2,g_cost2,g_cost1,tail,dup_q2,dup_q1,g_color1,GRAY0);
					cudaError_t err;
					err=cudaGetLastError();
			...
			if (num_of_blocks%2==0){
				 ...
				 dup_q2=d_q1;
					...
					pthread_barrier_wait(&bar);
                                      ...
                                      dim3  grid1( ceil((int)num_t2/(double)MAX_THREADS_PER_BLOCK), 1, 1);
					printf("numt1=%d  %d ",num_t2,grid1.x);
                                  //      dim3  threads1(MAX_THREADS_PER_BLOCK, 1, 1);
		...
		//			cudaMemcpyHostToDevice));
				...
				grid= dim3((num_of_blocks/2), 1, 1);
				 ...
				 dup_q4=d_q2;
					...
					printf("numt1=%d  %d ",num_t1,grid1.x);
                                  //      dim3  threads1(MAX_THREADS_PER_BLOCK, 1, 1);
