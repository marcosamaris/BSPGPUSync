...
inline bool  BFS_GPU( Node * d_graph_nodes,Edge * d_graph_edges,
		...
		if(k%2 == 0){
			...
			//	STARTt_TIMER;
				...
				if (num_of_blocks%2==0){
					...
					BFS_kernel3<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes,
							d_graph_edges, d_color, d_cost, num_t, tail, GRAY0,k);
					cudaDeviceSynchronize();
					...
					cudaMemcpyPeer(g_cost2,0,g_cost3,1,no_of_nodes*sizeof(int));
					cudaDeviceSynchronize();
					(cudaMemcpy(&num_t1,tail,sizeof(int),cudaMemcpyDeviceToHost));
					pthread_barrier_wait(&bar);


					cudaMemcpyPeer(d_q2+num_t1,0,dup_q1,1,num_t2*sizeof(int));
					cudaDeviceSynchronize();
					int numt=num_t1+num_t2;
					cudaMemcpy(tail,&numt,sizeof(int),cudaMemcpyHostToDevice);
					grid= dim3((int)ceil(num_t2/(double)512), 1, 1);
					cost_adjust<<<grid,threads >>>(num_t2,num_t1,d_q2,g_cost2,g_cost1,g_color1,GRAY0);
					cudaDeviceSynchronize();
					//pthread_barrier_wait(&bar);
		
					//dim3  grid1((int) ceil(num_t2/(double)MAX_THREADS_PER_BLOCK), 1, 1);
					//printf("1 numt2=%d  num_t1=%d ",num_t2,num_t1);
					...
					BFS_kernel1<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes,
							d_graph_edges, d_color, d_cost, num_t, tail, GRAY0,k);
					cudaDeviceSynchronize();
					dup_q2=d_q2;
					dup_q3=d_q2;
					...
					cudaMemcpyPeer(g_cost2,0,g_cost3,1,no_of_nodes*sizeof(int));
					cudaDeviceSynchronize();
					//cost_adjust<<<grid,threads >>>(num_t-(num_of_blocks/2)*512,(num_of_blocks/2)*512,d_q1,g_cost2,g_cost1);
					//cudaDeviceSynchronize();
					(cudaMemcpy(&num_t1,tail1,sizeof(int),cudaMemcpyDeviceToHost));
					pthread_barrier_wait(&bar);
					...
					//dim3  grid1( (int)ceil(num_t2/(double)MAX_THREADS_PER_BLOCK), 1, 1);
					//printf("2 numt1=%d numt2= %d ",num_t1,num_t2);
					//duplicate_detection<<<grid1,threads>>>(num_t2,num_t1,g_cost2,g_cost1,tail,d_q2,g_color1,GRAY0);
					//cudaDeviceSynchronize();
			...
			//	STARTt_TIMER;
				...
				if (num_of_blocks%2==0){
					...
					grid= dim3((num_of_blocks/2), 1, 1);
					BFS_kernel3<<< grid, threads >>>(d_q2,d_q1, d_graph_nodes,
							d_graph_edges, d_color, d_cost, num_t, tail, GRAY1,k);
					cudaDeviceSynchronize();
					...
					cudaMemcpyPeer(g_cost2,0,g_cost3,1,no_of_nodes*sizeof(int));
					cudaDeviceSynchronize();
					//cost_adjust<<<grid,threads >>>(num_t-(num_of_blocks/2)*512,(num_of_blocks/2)*512,d_q2,g_cost2,g_cost1);
					//cudaDeviceSynchronize();
					(cudaMemcpy(&num_t1,tail1,sizeof(int),cudaMemcpyDeviceToHost));
					pthread_barrier_wait(&bar);

					cudaMemcpyPeer(d_q1+num_t1,0,dup_q1,1,num_t2*sizeof(int));
					cudaDeviceSynchronize();
					int numt=num_t1+num_t2;
					cudaMemcpy(tail,&numt,sizeof(int),cudaMemcpyHostToDevice);
					grid= dim3((int)ceil(num_t2/(double)512), 1, 1);
					...
					//dim3  grid1( (int)(ceil(num_t2/(double)MAX_THREADS_PER_BLOCK)), 1, 1);
					//printf("3 numt1=%d  numt2=%d ",num_t1,num_t2);
					...
					grid= dim3((int)ceil(num_of_blocks/(double)2), 1, 1);
					BFS_kernel1<<< grid, threads >>>(d_q2,d_q1, d_graph_nodes,
							d_graph_edges, d_color, d_cost, num_t, tail, GRAY1,k);
					cudaDeviceSynchronize();
					cudaError_t err;
					err=cudaGetLastError();
					if (err != cudaSuccess){
						printf("BFS1 error=%d ",err);
						exit(-1);
					}
					dup_q3=d_q1;
					dup_q2=d_q1;
					...
					int numt=num_t1+num_t2;
					cudaMemcpy(tail,&numt,sizeof(int),cudaMemcpyHostToDevice);
		//			int grd= ((int)ceil(num_t2/(double)512));
		//			printf(" grid size=%d", grd);
					grid= dim3((int)ceil(num_t2/(double)512), 1, 1);
					cost_adjust<<<grid,threads >>>(num_t2,num_t1,d_q1,g_cost2,g_cost1,g_color1,GRAY1);
					cudaDeviceSynchronize();
					//pthread_barrier_wait(&bar);
					...
					//dim3  grid1( (int)ceil(num_t2/(double)MAX_THREADS_PER_BLOCK), 1, 1);
		//		printf("4 numt1=%d  numt2=%d ",num_t1,num_t2);
					//duplicate_detection<<<grid1,threads>>>(num_t2,num_t1,g_cost2,g_cost1,tail,d_q1,g_color1,GRAY1);
					//cudaDeviceSynchronize();
...
inline bool  BFS_GPU2( Node * d_graph_nodes,Edge * d_graph_edges,
		int * d_color, int * d_cost, int * d_q1, int * d_q2, int * tail, int source, int & global_kt){
	int num_of_blocks; 
	int num_of_threads_per_block;
	cudaSetDevice(1);
		...
		if(k%2 == 0){
			...
			//	STARTt_TIMER;
				...
				if (num_of_blocks%2 == 0){
					...
					if (l == 0){
						cudaMemcpyPeer(d_q1,1,dup_q3,0,num_t*sizeof(int));
						cudaDeviceSynchronize();
						l++;
					}
					pthread_barrier_wait(&bar);
					...
					grid= dim3((num_of_blocks/2), 1, 1);
					BFS_kernel4<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes, 
							d_graph_edges, d_color, d_cost, num_t, tail, GRAY0,k);
					cudaDeviceSynchronize();
					dup_q1=d_q2;
					dup_q4=d_q2;
					...
					cudaMemcpyPeer(g_cost4,1,g_cost1,0,no_of_nodes*sizeof(int));
					cudaDeviceSynchronize();
					//cost_adjust<<<grid,threads >>>((num_of_blocks/2)*512,0,d_q1,g_cost4,g_cost3);
					//cudaDeviceSynchronize();
					(cudaMemcpy(&num_t2,tail2,sizeof(int),cudaMemcpyDeviceToHost));
					pthread_barrier_wait(&bar);

					cudaMemcpy(d_q1+num_t1,dup_q1,num_t2*sizeof(int),cudaMemcpyDeviceToDevice);
					cudaMemcpyPeer(d_q1,1,dup_q3,0,num_t1*sizeof(int));
					cudaDeviceSynchronize();
					int numt=num_t1+num_t2;
			/*	if (numt<5000){
					cudaMemcpy(h_cost,d_q2,numt*sizeof(int),cudaMemcpyDeviceToHost);
					int f,x,y;
					for(x=0; x<numt; x++)
					{
						for(y=0; y<numt-1; y++)
						{
							if(h_cost[y]>h_cost[y+1])
							{
								int temp = h_cost[y+1];
								h_cost[y+1] = h_cost[y];
								h_cost[y] = temp;
				...
				for(f=0;f<numt;f++){
					if (h_cost[f]==h_cost[f+1]){printf("fuck dupes\n"); exit(-1);}
					printf("d_q[%d]=%d\n",f,h_cost[f]);}
				sleep(100);
			}*/
					grid= dim3((int)ceil(num_t1/(double)512), 1, 1);
					cost_adjust<<<grid,threads >>>(num_t1,0,d_q1,g_cost4,g_cost3,g_color2,GRAY0);
					cudaDeviceSynchronize();
					cudaMemcpy(tail,&numt,sizeof(int),cudaMemcpyHostToDevice);
					//pthread_barrier_wait(&bar);

					//dim3  grid1( (int)ceil(num_t2/(double)MAX_THREADS_PER_BLOCK), 1, 1);
				//	printf("5 numt1=%d numt2= %d ",num_t1,num_t2);
					//duplicate_detection<<<grid1,threads>>>(num_t2,num_t1,g_cost4,g_cost3,tail,d_q1,g_color2,GRAY0);
				//	cudaDeviceSynchronize();
				...
				//	pthread_barrier_wait(&bar);
					...
					grid= dim3((int)ceil(num_of_blocks/(double)2), 1, 1);
					BFS_kernel2<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes, 
							d_graph_edges, d_color, d_cost, num_t, tail, GRAY0,k);
					cudaDeviceSynchronize();
					dup_q1=d_q2;
					dup_q4=d_q2;
			...
			//		dim3  grid1( (int)ceil(num_t2/(double)MAX_THREADS_PER_BLOCK), 1, 1);
				//	printf("6 numt1=%d numt2= %d ",num_t1,num_t2);
					//duplicate_detection<<<grid1,threads>>>(num_t2, num_t1,g_cost4,g_cost3,tail,d_q1,g_color2,GRAY0);
			//		cudaDeviceSynchronize();
			...
			//	STARTt_TIMER;
				...
				if (num_of_blocks%2 == 0){
					...
					pthread_barrier_wait(&bar);
					if (l == 0){
						cudaMemcpyPeer(d_q2,1,dup_q3,0,num_t*sizeof(int));
						cudaDeviceSynchronize();
						l++;
					}
					pthread_barrier_wait(&bar);
					...
					grid= dim3((num_of_blocks/2), 1, 1);
					BFS_kernel4<<< grid, threads >>>(d_q2,d_q1, d_graph_nodes, 
							d_graph_edges, d_color, d_cost, num_t, tail, GRAY1,k);
					cudaDeviceSynchronize();
					dup_q1=d_q1;
					dup_q4=d_q1;
					...
					cudaMemcpyPeer(g_cost4,1,g_cost1,0,no_of_nodes*sizeof(int));
					cudaDeviceSynchronize();
					//cost_adjust<<<grid,threads >>>((num_of_blocks/2)*512,0,d_q2,g_cost4,g_cost3);
					//cudaDeviceSynchronize();
					(cudaMemcpy(&num_t2,tail2,sizeof(int),cudaMemcpyDeviceToHost));
					pthread_barrier_wait(&bar);

					cudaMemcpy(d_q2+num_t1,dup_q1,num_t2*sizeof(int),cudaMemcpyDeviceToDevice);
					cudaMemcpyPeer(d_q2,1,dup_q3,0,num_t1*sizeof(int));
					cudaDeviceSynchronize();
					int numt=num_t1+num_t2;
					cudaMemcpy(tail,&numt,sizeof(int),cudaMemcpyHostToDevice);
					grid= dim3((int)ceil(num_t1/(double)512), 1, 1);
					cost_adjust<<<grid,threads >>>(num_t1,0,d_q2,g_cost4,g_cost3,g_color2,GRAY1);
					cudaDeviceSynchronize();
				//	pthread_barrier_wait(&bar);

				//	dim3  grid1((int) ceil(num_t2/(double)MAX_THREADS_PER_BLOCK), 1, 1);
			//		printf("7 numt1=%d numt2=%d",num_t1,num_t2);
					//duplicate_detection<<<grid1,threads>>>(num_t2,num_t1,g_cost4,g_cost3,tail,d_q2,g_color2,GRAY1);
				//	cudaDeviceSynchronize();
				...
				//	pthread_barrier_wait(&bar);
					...
					if (l == 0){
						...
						err=cudaGetLastError();
						if (err != cudaSuccess){
							printf("memcpy peer error=%d ",err);
							exit(-1);
						}
						l++;
					}
					...
					grid= dim3((int)ceil(num_of_blocks/(double)2), 1, 1);
					BFS_kernel2<<< grid, threads >>>(d_q2,d_q1, d_graph_nodes, 
							d_graph_edges, d_color, d_cost, num_t, tail, GRAY1,k);
					cudaDeviceSynchronize();
					err=cudaGetLastError();
					if (err != cudaSuccess){
						printf("BFS error=%d ",err);
						exit(-1);
					}
					dup_q1=d_q1;
					dup_q4=d_q1;
			...
			//		int grd= ((int)ceil(num_t1/(double)512));
			//		printf(" grid size=%d", grd);
					grid= dim3((int)ceil(num_t1/(double)512), 1, 1);
					cost_adjust<<<grid,threads >>>(num_t1,0,d_q2,g_cost4,g_cost3,g_color2,GRAY1);
					cudaDeviceSynchronize();
			...
			//	cudaMemcpyPeer(g_cost4,1,g_cost1,0,no_of_nodes*sizeof(int));
			//	dim3 grid2((int)ceil(numt/(double)MAX_THREADS_PER_BLOCK), 1, 1);
			//	printcost<<<grid2,threads>>>(d_q2,g_cost3,g_cost4);
			//	cudaMemcpy(tail,&numt,no_of_nodes*sizeof(int),cudaMemcpyDeviceToHost);
			//	cudaDeviceSynchronize();

			//	pthread_barrier_wait(&bar);
					
			//		dim3  grid1((int) ceil(num_t2/(double)MAX_THREADS_PER_BLOCK), 1, 1);
				//	printf("8  numt1=%d numt2= %d",num_t1,num_t2);
					//duplicate_detection<<<grid1,threads>>>(num_t2,num_t1,g_cost4,g_cost3,tail,d_q2,g_color2,GRAY1);
			//		cudaDeviceSynchronize();
				//	pthread_barrier_wait(&bar);
...
void runGPU( int argc, char** argv) 
{
	...
	for (int i=0;i<NUM_GPUS;i++){
		param[i].h_graph_nodes=h_graph_nodes;
		param[i].h_graph_edges=h_graph_edges;
...
void *multithreaded_bfs(void *paramt){
	...
	if (parameters->id ==0){
		...
		(cudaBindTexture(0,g_graph_node_ref,d_graph_nodes2, sizeof(Node)*no_of_nodes));
		(cudaBindTexture(0,g_graph_edge_ref,d_graph_edges2,sizeof(Edge)*edge_list_size));

		int cur_count = 0;
		printf("Starting GPU kernel1\n");
		(cudaDeviceSynchronize());
		STARTt_TIMER;
		BFS_GPU2( d_graph_nodes2,d_graph_edges2,
				g_color2, g_cost3, g_q3, g_q4,  tail2, source, cur_count);
		(cudaDeviceSynchronize());
		ENDt_TIMER("bfs");
		int rc = pthread_barrier_wait(&bar);
		if(rc != 0 && rc != PTHREAD_BARRIER_SERIAL_THREAD)
		...
		/*int x;
		  tail2=tail1;
		  cudaMemcpy(&x,tail2,sizeof(int),cudaMemcpyDeviceToHost);
		  printf("x=%d\n",x);
		 */
		( cudaMemcpy( h_cost, g_cost3, sizeof(int)*no_of_nodes, cudaMemcpyDeviceToHost) );
		( cudaMemcpy( color, g_color2, sizeof(int)*no_of_nodes, cudaMemcpyDeviceToHost) );
//		for (int i=0;i<no_of_nodes;i++)
//			printf("color[%d]=%d\n",color[i]);
		( cudaUnbindTexture(g_graph_node_ref) );
		( cudaUnbindTexture(g_graph_edge_ref) );
...
//		printf("BFS_LEVEL=%d elements=%d\n",global_kt,num_t);
		...
		if(k%2 == 0){
			...
			//	ENDt_TIMER("kernell1");
				...
				cudaSetDevice(0);
				(cudaMemcpy(tail1,&zero,sizeof(int),cudaMemcpyHostToDevice));
				cudaSetDevice(1);
				(cudaMemcpy(tail2,&zero,sizeof(int),cudaMemcpyHostToDevice));
				cudaMemcpyPeer(g_q3,1,g_q1,0,num_t*sizeof(int));	
				cudaMemcpyPeer(g_cost3,1,g_cost1,0,no_of_nodes*sizeof(int));
				cudaMemcpyPeer(g_color2,1,g_color1,0,no_of_nodes*sizeof(int));
					cudaDeviceSynchronize();
			//	STARTt_TIMER;
				...
				cudaSetDevice(1);
					BFS_kernel4<<< grid, threads >>>(g_q3,g_q4, d_graph_nodes2,
							d_graph_edges2, g_color2, g_cost3, num_t, tail2, GRAY0,k);
					cudaDeviceSynchronize();
					(cudaMemcpy(&num_t2,tail2,sizeof(int),cudaMemcpyDeviceToHost));
				cudaSetDevice(0);
					BFS_kernel3<<< grid, threads >>>(g_q1,g_q2, d_graph_nodes,
							d_graph_edges, g_color, g_cost1, num_t, tail1, GRAY0,k);
					cudaDeviceSynchronize();
					...
					cudaMemcpyPeer(g_q2+num_t1,0,g_q4,1,num_t2*sizeof(int));
					cudaDeviceSynchronize();
					...
					grid= dim3((int)ceil(num_of_blocks/(double)2), 1, 1);
				cudaSetDevice(1);
					BFS_kernel2<<< grid, threads >>>(g_q3,g_q4, d_graph_nodes2,
							d_graph_edges2, g_color2, g_cost3, num_t, tail2, GRAY0,k);
					cudaDeviceSynchronize();
					(cudaMemcpy(&num_t2,tail2,sizeof(int),cudaMemcpyDeviceToHost));
				cudaSetDevice(0);
					BFS_kernel1<<< grid, threads >>>(g_q1,g_q2, d_graph_nodes,
			...
			//	ENDt_TIMER("kernell2");
				...
				cudaSetDevice(1);
				(cudaMemcpy(tail2,&zero,sizeof(int),cudaMemcpyHostToDevice));
				cudaMemcpyPeer(g_q3,1,g_q2,0,num_t*sizeof(int));	
				cudaMemcpyPeer(g_cost3,1,g_cost1,0,no_of_nodes*sizeof(int));
				cudaMemcpyPeer(g_color2,1,g_color1,0,no_of_nodes*sizeof(int));
					cudaDeviceSynchronize();
			//	STARTt_TIMER;
					...
					grid= dim3((num_of_blocks/2), 1, 1);
				cudaSetDevice(1);
					BFS_kernel4<<< grid, threads >>>(g_q4,g_q3, d_graph_nodes2,
							d_graph_edges2, g_color2, g_cost3, num_t, tail2, GRAY0,k);
					cudaDeviceSynchronize();
					(cudaMemcpy(&num_t2,tail2,sizeof(int),cudaMemcpyDeviceToHost));
				cudaSetDevice(0);
					BFS_kernel3<<< grid, threads >>>(g_q2,g_q1, d_graph_nodes,
					...
					cudaMemcpyPeer(g_q1+num_t1,0,g_q3,1,num_t2*sizeof(int));
					cudaDeviceSynchronize();
					int numt=num_t1+num_t2;
					cudaMemcpy(tail1,&numt,sizeof(int),cudaMemcpyHostToDevice);
					grid= dim3((int)ceil(num_t2/(double)512), 1, 1);
					...
					grid= dim3((int)ceil(num_of_blocks/(double)2), 1, 1);
				cudaSetDevice(1);
					BFS_kernel2<<< grid, threads >>>(g_q4,g_q3, d_graph_nodes2,
							d_graph_edges2, g_color2, g_cost3, num_t, tail2, GRAY0,k);
					cudaDeviceSynchronize();
					(cudaMemcpy(&num_t2,tail2,sizeof(int),cudaMemcpyDeviceToHost));
				cudaSetDevice(0);
