...
//		printf("BFS_LEVEL=%d elements=%d\n",global_kt,num_t);
		...
		if(k%2 == 0){
			if(num_of_blocks == 1){ 
				...
				(cudaMemcpy(tail,&zero,sizeof(int),cudaMemcpyHostToDevice));
				//STARTt_TIMER;
				BFS_in_GPU_kernel1<<< grid, threads >>>(d_q1,d_q2, d_graph_nodes, 
						d_graph_edges, d_color, d_cost,num_t , tail,GRAY0,k );
				cudaDeviceSynchronize();
			//	ENDt_TIMER("kernelll1");
			}
			...
			if(num_of_blocks == 1){  
				...
				(cudaMemcpy(tail,&zero,sizeof(int),cudaMemcpyHostToDevice));
			//	STARTt_TIMER;
				BFS_in_GPU_kernel1<<< grid, threads >>>(d_q2,d_q1, d_graph_nodes, 
						d_graph_edges, d_color, d_cost, num_t, tail,GRAY1,k);
				cudaDeviceSynchronize();
			//	ENDt_TIMER("kernelll2");
			}
...
//printf("finished unbinding texture1\n");
...
//printf("finished unbinding texture2\n");
