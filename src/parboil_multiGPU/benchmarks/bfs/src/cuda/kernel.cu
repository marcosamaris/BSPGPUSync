...
texture<Node> g_graph_node_ref2;
texture<Edge> g_graph_edge_ref2;
...
//--------------------------------------------------
	__global__ void
BFS_in_GPU_kernel1( int * q1, int * q2, Node* g_graph_nodes, Edge* g_graph_edges, int* g_color, int * g_cost, int no_of_nodes, int * tail, int gray_shade, int k ) 
{
	...
	while(1){//propage through multiple BFS levels until the wavfront overgrows one-block limit
		...
		if( tid<no_of_nodes)
		{
			...
			Node cur_node = tex1Dfetch(g_graph_node_ref2,pid);
			for(int i=cur_node.x; i<cur_node.y + cur_node.x; i++) {
				Edge cur_edge = tex1Dfetch(g_graph_edge_ref2,i);
				int id = cur_edge.x;
				int cost = cur_edge.y;
				cost += cur_cost;
...
//--------------------------------------------------------------
	__global__ void
BFS_kernel_multi_blk_inGPU1( int *  q1, int *  q2, Node* g_graph_nodes, Edge* g_graph_edges, int* g_color, int * g_cost, int *no_of_nodes, int * tail, int gray_shade, int k,   
 ...
 ***********************************************************************/
	__global__ void
BFS_kernel1( int * q1, int * q2, Node* g_graph_nodes, Edge* g_graph_edges, int* g_color, int * g_cost, int no_of_nodes, int * tail, int gray_shade, int k ) 
{
	__shared__ int local_q_tail[NUM_BIN];//the tails of each local warp-level queue
	__shared__ int local_q[NUM_BIN][LOCAL_MEM];//the local warp-level queues 
	__shared__ int prefix_q[NUM_BIN];//the number of elementss in the w-queues ahead of
	...
	//int bid= blockDim.x *(gridDim.x-1);
	int tid = blockIdx.x*MAX_THREADS_PER_BLOCK + threadIdx.x;
	//int ttid=tid+bid;
	//if( tid<no_of_nodes)
...
//	{
	...
	__syncthreads();
	//	if (no_of_nodes&511
	//int no=gridDim.x no_of_nodes/2;
	//first, propagate and add the new frontier elements into w-queues
		int pid = q1[tid]; //the current frontier node, or the parent node of the new frontier nodes 
		g_color[pid] = BLACK;
//		int ppid=q1[tid+bid];
...
//		__syncthreads();
	...
	//			g_color[no_of_nodes-1] = BLACK;
		...
		q_i = threadIdx.x&MOD_OP;//w-queue index
		int local_shift = threadIdx.x>>EXP;//shift within a w-queue
...
//		__syncthreads();
	...
	__global__ void
BFS_kernel2( int * q1, int * q2, Node* g_graph_nodes, Edge* g_graph_edges, int* g_color, int * g_cost, int no_of_nodes, int * tail, int gray_shade, int k ) 
{
	__shared__ int local_q_tail[NUM_BIN];//the tails of each local warp-level queue
	__shared__ int local_q[NUM_BIN][LOCAL_MEM];//the local warp-level queues 
	__shared__ int prefix_q[NUM_BIN];//the number of elementss in the w-queues ahead of
	...
	__syncthreads();
	//	int no=no_of_nodes/2;
	//first, propagate and add the new frontier elements into w-queues
//	int bid= blockDim.x *(gridDim.x-1);
	int ttid = blockIdx.x*MAX_THREADS_PER_BLOCK + threadIdx.x+blockDim.x *(gridDim.x-1);
//	int ttid=tid+bid;
/*	if (blockIdx.x <(gridDim.x -1)){
			int ppid=q1[tid];
			g_color[ppid] = BLACK;
	...
	if( ttid<no_of_nodes)
	{
		int pid = q1[ttid]; //the current frontier node, or the parent node of the new frontier nodes 
		g_color[pid] = BLACK;
		int cur_cost = g_cost[pid];
		int q_i = threadIdx.x&MOD_OP; //the id of the queue which new frontier nodes will be pushed
		//into
	...
	__global__ void
BFS_kernel3( int * q1, int * q2, Node* g_graph_nodes, Edge* g_graph_edges, int* g_color, int * g_cost, int no_of_nodes, int * tail, int gray_shade, int k ) 
{
	__shared__ int local_q_tail[NUM_BIN];//the tails of each local warp-level queue
	__shared__ int local_q[NUM_BIN][LOCAL_MEM];//the local warp-level queues 
	__shared__ int prefix_q[NUM_BIN];//the number of elementss in the w-queues ahead of
...
//}
	...
	__global__ void
BFS_kernel4( int * q1, int * q2, Node* g_graph_nodes, Edge* g_graph_edges, int* g_color, int * g_cost, int no_of_nodes, int * tail, int gray_shade, int k ) 
{
	__shared__ int local_q_tail[NUM_BIN];//the tails of each local warp-level queue
	__shared__ int local_q[NUM_BIN][LOCAL_MEM];//the local warp-level queues 
	__shared__ int prefix_q[NUM_BIN];//the number of elementss in the w-queues ahead of
	...
	//first, propagate and add the new frontier elements into w-queues
//	int bid= blockDim.x *gridDim.x;
	int ttid = blockIdx.x*MAX_THREADS_PER_BLOCK + threadIdx.x+blockDim.x *gridDim.x;
//	int ttid=tid+bid;
	if( ttid<no_of_nodes)
	{
		int pid = q1[ttid]; //the current frontier node, or the parent node of the new frontier nodes 
...
__global__ void duplicate_detection(int numt,int num_tt, int *d_cost1, int *d_cost2,int *taill, int *d_q1, int *d_color2,int grey_shade){
	int tid,node,cost1,cost2;
	tid=blockIdx.x*blockDim.x + threadIdx.x;
//	if (threadIdx.x==0 && blockIdx.x==0)
//	printf("numt=%d num_tt=%d tail=%d",numt,num_tt,taill[0]);
//	__syncthreads();
//	if (blockIdx.x <100)
//		printf("inumt =%d %d %d\n",blockDim.x,blockIdx.x,tid);
//	__syncthreads();
	if (tid <numt)
	{	
		node =d_q1[tid+num_tt];//q1 remote queue
		//printf("%d ",node);
		cost1=d_cost1[node];//cost1 rmote cost
		cost2=d_cost2[node];

	//	printf("inumt =%d %d cost1=%d cost2=%d node=%d\n",blockDim.x,blockIdx.x,cost1,cost2,node);
		if (cost1<=cost2){
			if (cost2 !=INF){
			int new_tail = atomicSub(taill,1);
	//		printf("tail=%d cost=%d color=%d\n",new_tail,cost1, d_color2[node]);
			d_q1[num_tt+tid]=d_q1[new_tail];

			}
			else if (cost1<cost2){
	//		printf("tail=%d cost=%d color=%d\n",new_tail,cost1, d_color2[node]);
				d_color2[node]=grey_shade;
				d_cost2[node]=cost1;
			}
		}
	}
...
__global__ void printcost(int * dq,int* costt1, int * costt2){

int tid= threadIdx.x+blockIdx.x*blockDim.x;
int node = dq[tid];
printf("cost1= %d, cost2=%d\n",costt1[node],costt2[node]);


}
__global__ void cost_adjust(int numt,int num_tt, int *d_q1,int *d_cost1, int *d_cost2, int *d_colour, int greyshade){
	...
	if (tid<numt)
	{
		node=d_q1[tid+num_tt];
//		if (d_cost1[node] ==INF)
//			printf("tid=%d cost1=%d cost2=%d\n",tid, d_cost1[node], d_cost2[node]);
		d_cost2[node]=d_cost1[node];
		d_colour[node]=greyshade;
	}
	__syncthreads();
