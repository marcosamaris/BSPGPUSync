		...
		if(k%2 == 0){
			if(num_of_blocks == 1){ 
				...
				if (num_of_blocks%2==0){
					...
					//      dim3  threads1(MAX_THREADS_PER_BLOCK, 1, 1);
					duplicate_detection<<<grid1,threads>>>(num_t2,g_cost2,g_cost1,tail,dup_q2,dup_q1,g_color1,GRAY0);
					cudaError_t err;
					...
					//BFS_kernel<<< grid/2, threads >>>(d_q2+num_t/2,d_q1, d_graph_nodes, 
					//       d_graph_edges, d_color, d_cost, num_t/2+1, tail, GRAY1,k);
			...
			if(num_of_blocks == 1){  
				...
				if (num_of_blocks%2==0){
					...
					//      dim3  threads1(MAX_THREADS_PER_BLOCK, 1, 1);
					duplicate_detection<<<grid1,threads>>>(num_t2,g_cost2,g_cost1,tail,dup_q2,dup_q1,g_color1,GRAY1);
					cudaError_t err;
					err=cudaGetLastError();
					if (err != cudaSuccess){
						printf("duplicate detectin kernel error =%d ",err);
		...
		if(k%2 == 0){
			if(num_of_blocks == 1){ 
				...
				if (num_of_blocks%2==0){
					...
					pthread_barrier_wait(&bar);
					//(cudaMemcpy(&num_t1,tail1,sizeof(int),cudaMemcpyDeviceToHost));
					dim3  grid1( (int)ceil(num_t1/(double)MAX_THREADS_PER_BLOCK), 1, 1);
					printf("5 numt1=%d numt2= %d ",num_t1,num_t2);
					//      dim3  threads1(MAX_THREADS_PER_BLOCK, 1, 1);
					duplicate_detection<<<grid1,threads>>>(num_t1,g_cost1,g_cost2,tail,dup_q4,dup_q3,g_color2,GRAY0);
					cudaError_t err;
					err=cudaGetLastError();
					if (err != cudaSuccess){
						printf("duplicate detectin kernel error=%d ",err);
			...
			if(num_of_blocks == 1){  
				...
				if (num_of_blocks%2==0){
					...
					//      dim3  threads1(MAX_THREADS_PER_BLOCK, 1, 1);
					duplicate_detection<<<grid1,threads>>>(num_t1,g_cost1,g_cost2,tail,dup_q4,dup_q3,g_color2,GRAY1);
					cudaError_t err;
					err=cudaGetLastError();
					if (err != cudaSuccess){
						printf("duplicate detectin kernel error=%d ",err);
...
void *multithreaded_bfs(void *paramt){
	...
	if (parameters->id ==0){
		...
		( cudaMemcpyAsync( g_cost2, h_cost, sizeof(int)*no_of_nodes, cudaMemcpyHostToDevice,stream2 ));
		...
		BFS_GPU2( d_graph_nodes2,d_graph_edges2,
				g_color2, g_cost2, g_q3, g_q4,  tail2, source, cur_count);
		(cudaDeviceSynchronize());
		...
		/*int x;
		  ...
		  printf("x=%d\n",x);
		 */
		( cudaMemcpy( h_cost, g_cost2, sizeof(int)*no_of_nodes, cudaMemcpyDeviceToHost) );
