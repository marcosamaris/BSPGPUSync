...
//					cudaMemcpyHostToDevice));
				...
				if (num_t%2==0){
					...
					(cudaMemcpy(&num_t1,tail1,sizeof(int),cudaMemcpyDeviceToHost));
	        			dim3  grid1( ceil(num_t1/512), 1, 1);
					dim3  threads1( 512, 1, 1);
					//duplicate_detection<<<grid1,threads1>>>(int num_t1);
...
void *multithreaded_bfs(void *paramt){
	...
	( cudaMalloc( (void**) &num_t2, sizeof(int)));
	...
	( cudaMemcpyAsync( d_color, color, sizeof(int)*no_of_nodes, cudaMemcpyHostToDevice,stream1 ));
	( cudaMemcpyAsync( d_cost, h_cost, sizeof(int)*no_of_nodes, cudaMemcpyHostToDevice,stream1 ));
...
printf("Starting GPU kernel0\n");
	...
	BFS_GPU( d_graph_nodes,d_graph_edges,
		d_color, d_cost, d_q1, d_q2,  tail1, source, cur_count);
	(cudaDeviceSynchronize());
...
//	( cudaMemcpy( color, d_color, sizeof(int)*no_of_nodes, cudaMemcpyDeviceToHost) );
	...
	( cudaMalloc( (void**) &d_color2, sizeof(int)*no_of_nodes) );
	( cudaMalloc( (void**) &d_cost2, sizeof(int)*no_of_nodes));
	( cudaMalloc( (void**) &d_q3, sizeof(int)*no_of_nodes));
	( cudaMalloc( (void**) &d_q4, sizeof(int)*no_of_nodes));
	( cudaMalloc( (void**) &tail2, sizeof(int)));
	( cudaMalloc( (void**) &num_t1, sizeof(int)));
	...
	( cudaMemcpyAsync( d_color2, color, sizeof(int)*no_of_nodes, cudaMemcpyHostToDevice,stream2 ));
	( cudaMemcpyAsync( d_cost2, h_cost, sizeof(int)*no_of_nodes, cudaMemcpyHostToDevice,stream2 ));
...
printf("Starting GPU kernel1\n");
	...
	BFS_GPU2( d_graph_nodes2,d_graph_edges2,
		d_color2, d_cost2, d_q3, d_q4,  tail2, source, cur_count);
	(cudaDeviceSynchronize());
...
printf("x=%d\n",x);
*/
	( cudaMemcpy( h_cost, d_cost2, sizeof(int)*no_of_nodes, cudaMemcpyDeviceToHost) );
	( cudaMemcpy( color, d_color2, sizeof(int)*no_of_nodes, cudaMemcpyDeviceToHost) );
