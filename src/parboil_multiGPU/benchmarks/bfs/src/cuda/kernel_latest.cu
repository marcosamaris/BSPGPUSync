...
__global__ void duplicate_detection(int num_tt,int *d_cost1, int *d_cost2,int *taill, int *d_q2, int *d_q1, int *d_color2,int grey_shade){
	int tid,node,cost1,cost2;
	tid=blockIdx.x*blockDim.x + threadIdx.x;
	if (threadIdx.x==0 && blockIdx.x==0)
	printf("numt=%d ",num_tt);
	__syncthreads();
	if (tid <num_tt)
	{	
		node =d_q1[tid];//q1 remote queue
		//printf("%d ",node);
