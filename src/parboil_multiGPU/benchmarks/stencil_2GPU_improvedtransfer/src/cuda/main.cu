...
//STARTt_TIMER;
	...
	cudaSetDevice(0);
		...
		block2D_hybrid_coarsen_x1<<<grid, block,sh_size>>>(c0,c1, d_A1, d_Anext1, nx, ny,  nz/2, cur_d_Br1,cur_d_Bw1,1);
//	 cudaDeviceSynchronize();
	cudaSetDevice(1);
	//	if (t%2==0){cur_d_Br0=d_A0; cur_d_Bw0=d_B1;}
	//	else	   {cur_d_Br0=d_B0;     cur_d_Bw0=d_Anext1;}
		cur_d_Br0=d_A0;cur_d_Bw0=d_Anext1;
		block2D_hybrid_coarsen_x2<<<grid, block,sh_size>>>(c0,c1, d_A0, d_Anext0, nx, ny,  nz/2, cur_d_Br0,cur_d_Bw0,1);
//	 cudaDeviceSynchronize();
...
//	cudaSetDevice(1);
	...
	cudaSetDevice(0);
	cudaMemcpy(h_Anext0+size/2, d_A1+nx*ny/*+size/2*/,size/2*sizeof(float), cudaMemcpyDeviceToHost);
//	if (parameters->synchronizeGpu) cudaThreadSynchronize();
	cudaSetDevice(1);
	cudaFree(d_A0);
    cudaFree(d_Anext0);
