...
h_next = tmp;
	...
	cudaMemset(d_Anext0,0,(nx*ny*(nz/3)+2*nx*ny)*sizeof(float));
...
//STARTt_TIMER;
	...
	{
	cudaSetDevice(3);
		block2D_hybrid_coarsen_middle<<<grid, block,sh_size>>>(c0,c1, d_A2, d_Anext2, nx, ny,  nz/3, d_Anext0,d_Anext1,1);

	cudaSetDevice(0);
		
		cur_d_Br1=d_A1;cur_d_Bw1=d_Anext2;
		block2D_hybrid_coarsen_last<<<grid, block,sh_size>>>(c0,c1, d_A1, d_Anext1, nx, ny,  nz/3, cur_d_Br1,cur_d_Bw1,nz-2*nz/3);
	
	cudaSetDevice(2);
		cur_d_Br0=d_A0;cur_d_Bw0=d_Anext2;
		block2D_hybrid_coarsen_first<<<grid, block,sh_size>>>(c0,c1, d_A0, d_Anext0, nx, ny,  nz/3, cur_d_Br0,cur_d_Bw0,1);
		
		cudaMemcpyPeer(d_B0,0,d_Anext2,1,sizeof(float));
		cudaMemcpyPeer(d_B1,0,d_Anext2,1,sizeof(float));
	//	cudaMemcpyPeer(d_B1,1,d_Anext0+nx*ny*(nz/2-1),0,sizeof(float));
	temp1= d_Anext0;
	d_Anext0=d_A0;
	d_A0=temp1;
...
//ENDt_TIMER("kernel");
	...
	cudaSetDevice(0);
	cudaMemcpy(h_Anext0+2*size/3, d_A1/*+size/2*/,(size-2*nx*ny*(nz/3))*sizeof(float), cudaMemcpyDeviceToHost);
	cudaSetDevice(3);
	cudaMemcpy(h_Anext0+size/3, d_A2/*+size/2*/,nx*ny*(nz/3)*sizeof(float), cudaMemcpyDeviceToHost);
//	if (parameters->synchronizeGpu) cudaThreadSynchronize();
