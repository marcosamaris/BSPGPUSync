...
__global__ void block2D_hybrid_coarsen_last(float c0,float c1,float *A0,float *Anext, int nx, int ny, int nz, float *d_Br, float *d_Bw, int nz2)
{
	...
	if((i<nx) &&(j<ny))
	{
		bottom=d_Br[Index3D (nx, ny, i, j, 0)];
		sh_A0[sh_id]=A0[Index3D (nx, ny, i, j, 1)];
	}
	if((i2<nx) &&(j<ny))
	{
		bottom2=d_Br[Index3D (nx, ny, i2, j, 0)];
...
__global__ void block2D_hybrid_coarsen_first(float c0,float c1,float *A0,float *Anext, int nx, int ny, int nz, float *d_Br, float *d_Bw,int flag)
{
	...
	if((i<nx) &&(j<ny))
	{
		...
		//sh_A0[sh_id]=A0[Index3D (nx, ny, i, j, 1)];
		bottom=d_Br[Index3D (nx, ny, i, j, nz)];
		sh_A0[sh_id]=A0[Index3D (nx, ny, i, j, nz-1)];
	}
	if((i2<nx) &&(j<ny))
	{
		bottom2=d_Br[Index3D (nx, ny, i2, j, nz)];
		sh_A0[sh_id2]=A0[Index3D (nx, ny, i2, j, nz-1)];
...
__global__ void block2D_hybrid_coarsen_middle(float c0,float c1,float *A0,float *Anext, int nx, int ny, int nz, float *d_Bw1, float *d_Bw2,int nz2)
{
	...
	if((i<nx) &&(j<ny))
	{
		//bottom=A0[Index3D (nx, ny, i, j, 0)];
		//sh_A0[sh_id]=A0[Index3D (nx, ny, i, j, 1)];
		bottom=A0[Index3D (nx, ny, i, j, nz)];
		sh_A0[sh_id]=A0[Index3D (nx, ny, i, j, nz-1)];
	}
	if((i2<nx) &&(j<ny))
	{
		bottom2=A0[Index3D (nx, ny, i2, j, nz)];
		sh_A0[sh_id2]=A0[Index3D (nx, ny, i2, j, nz-1)];
		//bottom2=A0[Index3D (nx, ny, i2, j, 0)];
		//sh_A0[sh_id2]=A0[Index3D (nx, ny, i2, j, 1)];
	}
