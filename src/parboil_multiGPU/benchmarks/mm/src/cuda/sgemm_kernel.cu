...
__global__ void mysgemmNT( const float *A, int lda, const float *B, int ldb, float* C, int ldc, int k, float alpha, float beta )
{
    ...
    int n = blockIdx.y * TILE_N + threadIdx.x;//addresses a 16X16 tile
    __shared__ float b_s[TILE_TB_HEIGHT][TILE_N];
    for (int i = 0; i < k; i+=TILE_TB_HEIGHT) {
	float a;
	// B is stored in shared memory in row-major format 
	b_s[threadIdx.y][threadIdx.x]=B[n + (i+threadIdx.y)*ldb];
	__syncthreads();
	for (int j = 0; j < TILE_TB_HEIGHT; j++) {
	    a = A[m + (i+j)*lda];//each thread reads 16 elements from a row
	    for (int kk = 0; kk < TILE_N; kk++)
		c[kk] += a * b_s[j][kk];//each thread does 16 *8 multipications to generate 16 output

	}
	__syncthreads();
    }

    //every thread writes 16 elements of a row of C
