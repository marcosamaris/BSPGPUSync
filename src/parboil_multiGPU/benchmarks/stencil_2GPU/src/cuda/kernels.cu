...
__global__ void block2D_hybrid_coarsen_x1(float c0,float c1,float *A0,float *Anext, int nx, int ny, int nz, float *d_Br, float *d_Bw, int flag)
{
	...
	if((i<nx) &&(j<ny))
	{
		bottom=d_Br[Index3D (nx, ny, i, j, (nz/2-1))];
		sh_A0[sh_id]=A0[Index3D (nx, ny, i, j, nz/2)];
	}
	if((i2<nx) &&(j<ny))
	{
		bottom2=d_Br[Index3D (nx, ny, i2, j, (nz/2-1))];
	...
	for(int k=nz/2;k<nz-1;k++)
	{
		...
		if(w_region)
		{
			...
			if (k==nz/2)
			d_Bw[Index3D (nx, ny, i, j, k)]=Anext[Index3D (nx, ny, i, j, k)] ;
		...
		if(w_region2)
		{
			...
			if (k==nz/2)
			d_Bw[Index3D (nx, ny, i2, j, k)]=Anext[Index3D (nx, ny, i2, j, k)] ;
		}
...
__global__ void block2D_hybrid_coarsen_x2(float c0,float c1,float *A0,float *Anext, int nx, int ny, int nz, float *d_Br, float *d_Bw,int flag)
{
	...
	if((i<nx) &&(j<ny))
	{
		...
		bottom=d_Br[Index3D (nx, ny, i, j, nz/2)];
		sh_A0[sh_id]=A0[Index3D (nx, ny, i, j, nz/2-1)];
	}
	if((i2<nx) &&(j<ny))
	{
		bottom2=d_Br[Index3D (nx, ny, i2, j, nz/2)];
		sh_A0[sh_id2]=A0[Index3D (nx, ny, i2, j, nz/2-1)];
