...
h_next = tmp;
	...
	cudaSetDevice(0);
	cudaMalloc((void**)&d_B1,sizeof(float)*nx*ny);	
	cudaMemset(d_B1,0,nx*ny*sizeof(float));
	pb_SwitchToTimer(&timers, pb_TimerID_COPY);

	//memory allocation
	cudaMalloc((void **)&d_A1, size*sizeof(float));
	cudaMalloc((void **)&d_Anext1, size*sizeof(float));
	cudaMemset(d_Anext1,0,size*sizeof(float));
	...
	cudaSetDevice(1);
	cudaMalloc((void**)&d_B0,sizeof(float)*nx*ny);	
	cudaMemset(d_B0,0,nx*ny*sizeof(float));
	h_Anext0=(float*)malloc(sizeof(float)*size);
	
	pb_SwitchToTimer(&timers, pb_TimerID_COPY);
	//memory allocation
	cudaMalloc((void **)&d_A0, size*sizeof(float));
	cudaMalloc((void **)&d_Anext0, size*sizeof(float));
	cudaMemset(d_Anext0,0,size*sizeof(float));

	//memory copy
	cudaMemcpy(d_A0, h_A, size*sizeof(float), cudaMemcpyHostToDevice);
...
//STARTt_TIMER;
	...
	{
	cudaSetDevice(0);
		//if (t%2==0){cur_d_Br1=d_A1; cur_d_Bw1=d_B0;}
		//else	   {cur_d_Br1=d_B1;     cur_d_Bw1=d_Anext0;}
		cur_d_Br1=d_A1;cur_d_Bw1=d_Anext0;
		block2D_hybrid_coarsen_x1<<<grid, block,sh_size>>>(c0,c1, d_A1, d_Anext1, nx, ny,  nz, cur_d_Br1,cur_d_Bw1,1);
//	 cudaDeviceSynchronize();
	cudaSetDevice(1);
	//	if (t%2==0){cur_d_Br0=d_A0; cur_d_Bw0=d_B1;}
	//	else	   {cur_d_Br0=d_B0;     cur_d_Bw0=d_Anext1;}
		cur_d_Br0=d_A0;cur_d_Bw0=d_Anext1;
		block2D_hybrid_coarsen_x2<<<grid, block,sh_size>>>(c0,c1, d_A0, d_Anext0, nx, ny,  nz, cur_d_Br0,cur_d_Bw0,1);
//	 cudaDeviceSynchronize();
		cudaMemcpyPeer(d_B0,0,d_Anext1+size/2,1,sizeof(float));
...
//	cudaSetDevice(1);
	...
	cudaSetDevice(0);
	cudaMemcpy(h_Anext0+size/2, d_A1+size/2,size/2*sizeof(float), cudaMemcpyDeviceToHost);
//	if (parameters->synchronizeGpu) cudaThreadSynchronize();
	cudaSetDevice(1);
	cudaFree(d_A0);
    cudaFree(d_Anext0);
