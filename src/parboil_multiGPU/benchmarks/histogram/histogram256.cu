...
extern "C" void initHistogram256(uint count){
    ( cudaMalloc((void **)&d_PartialHistograms, count * HISTOGRAM256_BIN_COUNT * sizeof(uint)) );
}
...
/*extern "C" void initHistogram256(void *d_PartialHistograms, size_t size){
    ( cudaMalloc((void **)&d_PartialHistograms, size * HISTOGRAM256_BIN_COUNT * sizeof(uint)) );
}

//Internal memory deallocation
extern "C" void closeHistogram256(void *d_PartialHistograms){
    ( cudaFree(d_PartialHistograms) );
}*/
...
dim3 grid= dim3(partial_count,1,1);
    //histogram256Kernel<<<PARTIAL_HISTOGRAM256_COUNT, HISTOGRAM256_THREADBLOCK_SIZE>>>(
    histogram256Kernel<<<grid, HISTOGRAM256_THREADBLOCK_SIZE>>>(
        d_PartialHistograms,
        (uint *)d_Data,
...
extern "C" void histogram256(
    ...
    assert( byteCount % sizeof(uint) == 0 );
dim3 grid= dim3(partial_count,1,1);
    histogram256Kernel<<<grid, HISTOGRAM256_THREADBLOCK_SIZE>>>(d_PartialHistograms,(uint *)d_Data,byteCount / sizeof(uint));
