...
//pthread parameters
#define NUM_GPUS 4
void *multithreaded_cutcp(void *parameter);
...
struct thread_params param[NUM_GPUS+1];
...
//cudaSetDevice(0);
  //cudaHostAlloc((void **)&binBaseAddr,(nbins * BIN_DEPTH, sizeof(float4)),cudaHostAllocPortable);
  //cudaMallocHost((void **)&binBaseAddr,(nbins * BIN_DEPTH, sizeof(float4)));
  //memset(binBaseAddr,0,(nbins * BIN_DEPTH, sizeof(float4)));
  CUERR;
...
//pb_SwitchToTimer( &timers, pb_TimerID_GPU );
   for(t=0; t<NUM_GPUS+1; t++) {

     fprintf(stderr,"Main: creating thread %ld\n", t);

        param[t].id=t;
	param[t].binBaseAddr=binBaseAddr;
	param[t].nbrlist=nbrlist;
	param[t].nbrlistlen=nbrlistlen;
	param[t].nbins=nbins;
	param[t].c=c;
	param[t].h=h;
	param[t].binDim=binDim;
	param[t].lnall=lnall;
	param[t].cutoff2=cutoff2;
	param[t].inv_cutoff2=inv_cutoff2;
	param[t].xRegionDim=xRegionDim;
	param[t].yRegionDim=yRegionDim;
	param[t].zRegionDim=zRegionDim;
	param[t].regionZeroAddr=regionZeroAddr;
	if (t == NUM_GPUS){
	param[t].extra=extra;
	param[t].lattice=lattice;
	param[t].cutoff=cutoff;

	}
      rc = pthread_create(&thread[t], &attr,multithreaded_cutcp,(void *) &param[t]);
...
cudaSetDevice(parameters->id);
  ...
  cudaMalloc((void **) &regionZeroCuda, parameters->lnall * sizeof(ener_t));
  //CUERR;
  cudaMemset(regionZeroCuda, 0, parameters->lnall * sizeof(ener_t));
  //CUERR;
  cudaMalloc((void **) &binBaseCuda, parameters->nbins * BIN_DEPTH * sizeof(float4));
  //CUERR;
  //cudaMemcpy(binBaseCuda, parameters->binBaseAddr, parameters->nbins * BIN_DEPTH * sizeof(float4),
    //  cudaMemcpyHostToDevice);
  cudaMemcpyAsync(binBaseCuda, parameters->binBaseAddr, parameters-> nbins * BIN_DEPTH * sizeof(float4), cudaMemcpyHostToDevice,cutoffstream);
  //CUERR;

binZeroCuda = binBaseCuda + ((parameters->c * (parameters-> binDim).y + parameters->c) * (parameters-> binDim).x + parameters->c) * BIN_DEPTH;
  cudaMemcpyToSymbol(NbrListLen,  &parameters->nbrlistlen, sizeof(int));
  //CUERR;
  cudaMemcpyToSymbol(NbrList,  parameters->nbrlist, parameters->nbrlistlen * sizeof(int3), 0);
  //CUERR;
cudaDeviceSynchronize();

  /* loop over z-dimension, invoke CUDA kernel for each x-y plane */
  //CUERR;
  printf("Invoking CUDA kernel on %d region planes...\n", parameters->zRegionDim);

int initial_index,final_index;

	initial_index= parameters->id * (parameters->zRegionDim/NUM_GPUS+ ((parameters->zRegionDim % NUM_GPUS >0) ? 1: 0));
if (parameters->id < NUM_GPUS-1){
	final_index = (parameters->id + 1) * (parameters->zRegionDim/NUM_GPUS+ ((parameters->zRegionDim % NUM_GPUS >0) ? 1: 0)); 
}
else{
	//initial_index= zRegionDim - ((NUM_GPUS-1)*((parameters->zRegionDim/NUM_GPUS) + (parameters->zRegionDim%NUM_GPUS?1:0)));
	final_index =parameters->zRegionDim;
}

  //  printf("  index %d %d\n", initial_index,final_index);
  for (int zRegionIndex = initial_index;  zRegionIndex < final_index;  zRegionIndex++) {
    printf("  computing plane %d\r", final_index-initial_index);
    fflush(stdout);
    cuda_cutoff_potential_lattice6overlap<<<gridDim, blockDim, 0,cutoffstream>>>((parameters->binDim).x, (parameters->binDim).y,
        binZeroCuda, parameters->h, parameters->cutoff2, parameters->inv_cutoff2, regionZeroCuda, zRegionIndex);
  }
  //CUERR;
...
long offst;
offst=(parameters->lnall/parameters->zRegionDim) *(final_index-initial_index);
//    printf("  size %d %d\n",offst,(parameters->lnall/parameters->zRegionDim)*initial_index);
  

/* copy result regions from CUDA device */
  cudaMemcpy(parameters->regionZeroAddr+(parameters->lnall/parameters->zRegionDim)*initial_index, regionZeroCuda+(parameters->lnall/parameters->zRegionDim)*initial_index, offst * sizeof(ener_t),
   ...
   */
//  pb_SwitchToTimer(timers, pb_TimerID_COMPUTE);
  if (parameters->extra->size > 0) {
    printf("computing extra atoms on CPU\n");
    if (cpu_compute_cutoff_potential_lattice(parameters->lattice, parameters->cutoff, parameters->extra)) {
