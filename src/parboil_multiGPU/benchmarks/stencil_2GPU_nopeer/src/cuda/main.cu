...
*/
	...
	float *h_Anext0;
	//device
	float *d_A0,*d_A1,*d_A2;
	float *d_Anext0,*d_Anext1,*d_Anext2;
...
cudaStream_t stream0, stream1,stream2;	
	...
	//memory allocation
	cudaMalloc((void **)&d_A1, (size-2*nx*ny*(nz/3)+nx*ny)*sizeof(float));
	cudaMalloc((void **)&d_Anext1, (size-2*nx*ny*(nz/3)+nx*ny)*sizeof(float));
	cudaMemset(d_Anext1,0,(size-2*nx*ny*(nz/3)+nx*ny)*sizeof(float));

	//memory copy
	cudaMemcpy(d_A1, h_A+2*nx*ny*(nz/3)-nx*ny, (size-2*nx*ny*(nz/3)+nx*ny)*sizeof(float), cudaMemcpyHostToDevice);
	...
	pb_SwitchToTimer(&timers, pb_TimerID_COPY);
	//memory allocation
	cudaMalloc((void **)&d_A0, (nx*ny*(nz/3)+nx*ny)*sizeof(float));
	cudaMalloc((void **)&d_Anext0, (nx*ny*(nz/3)+nx*ny)*sizeof(float));
	cudaMemset(d_Anext0,0,(nx*ny*(nz/3)+nx*ny)*sizeof(float));
	//memory copy
	cudaMemcpy(d_A0, h_A,(nx*ny*(nz/3)+nx*ny)*sizeof(float), cudaMemcpyHostToDevice);
	...
	pb_SwitchToTimer(&timers, pb_TimerID_COPY);
	//memory allocation
	cudaMalloc((void **)&d_A2, (nx*ny*(nz/3)+2*nx*ny)*sizeof(float));
	cudaMalloc((void **)&d_Anext2, (nx*ny*(nz/3)+2*nx*ny)*sizeof(float));
	cudaMemset(d_Anext2,0,(nx*ny*(nz/3)+2*nx*ny)*sizeof(float));

	//memory copy
	cudaMemcpy(d_A2, h_A+nx*ny*(nz/3)-nx*ny,(nx*ny*(nz/3)+2*nx*ny)*sizeof(float), cudaMemcpyHostToDevice);
...
//STARTt_TIMER;
	...
	{
	cudaSetDevice(2);
		block2D_hybrid_coarsen_interface<<<grid, block,sh_size,stream2>>>(c0,c1, d_A2, d_Anext2, nx, ny,  nz/3, d_A2,0,1);
		block2D_hybrid_coarsen_interface<<<grid, block,sh_size,stream2>>>(c0,c1, d_A2+nx*ny*((nz/3)-2), d_Anext2, nx, ny,  nz/3, d_A2,0,nz/3-1);
		cudaMemcpyAsync(d_Anext0+nx*ny*(nz/3),d_Anext2+nx*ny,nx*ny*sizeof(float),cudaMemcpyDefault,stream2);
		cudaMemcpyAsync(d_Anext1,d_Anext2+nx*ny*((nz/3)-1),nx*ny*sizeof(float),cudaMemcpyDefault,stream2);
	cudaSetDevice(1);
		block2D_hybrid_coarsen_interface<<<grid, block,sh_size,stream1>>>(c0,c1, d_A1, d_Anext1, nx, ny,  nz/3, d_A1,NULL,1);
		cudaMemcpyAsync(d_Anext2+nx*ny*(nz/3),d_Anext1+nx*ny,nx*ny*sizeof(float),cudaMemcpyDefault,stream1);
	cudaSetDevice(0);
		block2D_hybrid_coarsen_interface<<<grid, block,sh_size,stream0>>>(c0,c1, d_A0+nx*ny*((nz/3)-2), d_Anext0, nx, ny,  nz/3, d_A0,NULL,nz/3-1);
		cudaMemcpyAsync(d_Anext2,d_Anext0+nx*ny*((nz/3)-1),nx*ny*sizeof(float),cudaMemcpyDefault,stream0);

	cudaSetDevice(2);
		block2D_hybrid_coarsen_middle<<<grid, block,sh_size,stream2>>>(c0,c1, d_A2, d_Anext2, nx, ny,  nz/3, d_Anext0,d_Anext1,1);
	cudaSetDevice(1);
		
		cur_d_Br1=d_A1;cur_d_Bw1=d_Anext2;
		block2D_hybrid_coarsen_last<<<grid, block,sh_size,stream1>>>(c0,c1, d_A1, d_Anext1, nx, ny,  nz/3, cur_d_Br1,cur_d_Bw1,nz-2*(nz/3));
	
	cudaSetDevice(0);
		cur_d_Br0=d_A0;cur_d_Bw0=d_Anext2;
		block2D_hybrid_coarsen_first<<<grid, block,sh_size,stream0>>>(c0,c1, d_A0, d_Anext0, nx, ny,  nz/3, cur_d_Br0,cur_d_Bw0,1);
...
//ENDt_TIMER("kernel");
	...
	cudaSetDevice(0);
	pb_SwitchToTimer(&timers, pb_TimerID_COPY);
	cudaMemcpy(h_Anext0, d_A0,nx*ny*(nz/3)*sizeof(float), cudaMemcpyDeviceToHost);
	cudaSetDevice(1);
	cudaMemcpy(h_Anext0+2*nx*ny*(nz/3), d_A1+nx*ny,(size-2*nx*ny*(nz/3))*sizeof(float), cudaMemcpyDeviceToHost);
	cudaSetDevice(2);
	cudaMemcpy(h_Anext0+nx*ny*(nz/3), d_A2+nx*ny,nx*ny*(nz/3)*sizeof(float), cudaMemcpyDeviceToHost);
//	if (parameters->synchronizeGpu) cudaThreadSynchronize();
	cudaSetDevice(0);
	cudaFree(d_A0);
    cudaFree(d_Anext0);
