...
for app in "${apps[@]}"; do 
    ...
    if [[ "${app}" == "backprop" ]]; then
        for i in `seq 8192 1024 65536`; do
            nvprof   --print-gpu-trace --csv -u s ${execApps["backprop"]} $i 2> ../../../logs/${app}/${app}-$i.csv
        done
    fi
    ...
    if [[ "${app}" == "heartwall" ]]; then
        for i in `seq 20 104`; do
            nvprof   --print-gpu-trace --csv -u s ${execApps["heartwall"]} ../../data/heartwall/test.avi $i 2> ../../../logs/${app}/${app}-$i.csv
        done
    fi
    ...
    if [[ "${app}" == "lavaMD" ]]; then
        for i in `seq 5 1 100`; do
            nvprof   --print-gpu-trace --csv -u s ${execApps["lavaMD"]} -boxes1d $i 2> ../../../logs/${app}/${app}-$i.csv
        done
    fi
    ...
    if [[ "${app}" == "nw" ]]; then        
        for i in `seq 256 256 4096`; do
            for j in `seq 1 10 `; do
                nvprof   --print-gpu-trace --csv -u s ${execApps["nw"]} $i $j 2> ../../../logs/${app}/${app}-$i-$j.csv
            done
        done
    fi 
