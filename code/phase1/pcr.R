...
for(kernelApp in c(1:7, 9:13)) {
    ...
    for(gpu in c(1, 2, 3, 6, 7, 9, 10)){
        ...
        tempFeatures <- rbind(tempFeatures, tempAppGpu[sample(nrow(tempAppGpu), 57),])
    }
    ...
    for(iML in c("pcr")){
        for(threshCorr in c(0.5, 0.75)){
            ...
            Data <- cbind(tempDataset, duration=tempDuration, gpu_id=tempGpuData$gpu_id, flops=tempGpuData$theoretical_flops/max(tempGpuData$theoretical_flops), bw=tempGpuData$bandwith/max(tempGpuData$bandwith))
            
            if(length(tempDataset) > 9){
