        ...
        for(threshCorr in c(0.5)){
            ...
            if(length(tempData) > 10){
                ...
                for(numberFeatures in c(5, 10)){
                    ...
                    # if ( length(Data[names(Data[names(Data)  %in%  "issued_control.flow_instructions"])]) != 0)
                    # Data[names(Data[names(Data)  %in%  "issued_control.flow_instructions"])] <- normalizeLogMax(Data[names(Data[names(Data)  %in%  "issued_control.flow_instructions"])])
                    ...
                    for(gpu in c(1,2,3,5, 6,7,9,10)) {
                        ...
                        if (iML == "lm") fit <- fit <- lm(trainingDuration ~ ., data = trainingData)
                        ...
                        if (iML == "ann") fit <- nnet(trainingDuration ~ ., data = trainingData, size = 1, maxit=500, linout=TRUE, trace=FALSE)
