...
for(gpu in c(1, 2, 3, 5, 6, 7, 9, 10)) {
    ...
    for(iML in c("lm")){
        ...
        for(threshCorr in c(0.5, 0.75)){
            ...
            png(filename = paste("./images/phase2/correlation/heatMap-", gpus[gpu,'gpu_name'], " Thresh=", threshCorr, ".png", sep=""), width = 1600, height = 800)
            heatmap(x = cor(apply(tempData, 2, normalizeLogMax),
                            method = "spearman", use = "complete.obs"),
                    col = col, symm = TRUE)
            dev.off()
            
            png(filename = paste("./images/phase2/correlation/corClustring-", gpus[gpu,'gpu_name'], " Thresh=", threshCorr, ".png", sep=""), width = 1600, height = 800)
            ...
            if(length(tempData) > 10){
                ...
                for(numberFeatures in c(5, 10)){
                    ...
                    png(filename = paste("./images/phase2/cluster/", gpus[gpu,'gpu_name'], 
                    ...
                    Data <- data.frame(apply(Data, 2, normalizeLogMax), 
                                       duration=normalizeLogMax(tempDuration), 
                                       kernel=tempKernel)
                    
                    
                    png(filename = paste("./images/phase2/scatterPlot/", gpus[gpu,'gpu_name'], "-Thresh=", threshCorr, "-NParam=", numberFeatures, ".png", sep=""), width = 1600, height = 800)
                    scatterplotMatrix(Data,cex.labels =  1.5)
                    dev.off()
                    
                    meanAcc <- array()
                    png(filename = paste("./images/phase2/fitModels/", iML, "-", gpus[gpu,'gpu_name'], "-Thresh=", threshCorr, "-NParam=", numberFeatures, ".png", sep=""), width = 1600, height = 800)
                    ...
                    png(filename = paste("./images/phase2/features/",  gpus[gpu,'gpu_name'],"-Thresh_", threshCorr, "-NoFeatures_",numberFeatures, ".png", sep=""), width = 1200, height = 2800)
                    ...
                    if(numberFeatures ==  10){
                        ...
                        write.csv(cbind(c(names(Data), 0), meanAcc), file(paste("./results/phase2/", iML, "-",names(kernelsDict[kernelApp]), "-Thresh_", threshCorr, "-NoFeatures_",numberFeatures,".csv",sep="")))                
                    }
                }
            }
        }
        ...
        write.csv(Result, file(paste("./results/phase2/", iML, "-",names(kernelsDict[kernelApp]), ".csv",sep="")))
    }
}
