...
for(gpu in c(1, 2, 3, 5, 6, 7, 9, 10)) {
    for(kernelApp in c(1:7, 9:13)){
        ...
        tempAppGpu <- data.frame(cbind(fread(file = paste("./datasets/",names(kernelsDict[kernelApp]), "-", gpus[gpu,'gpu_name'], ".csv", sep=""),check.names = TRUE), kernel = kernelApp, gpus[gpu,]))        
...
tempFeatures <- tempFeatures[,!names(tempFeatures) %in% c(names(gpus))]
    ...
    tempFeatures$device_memory_read_transactions[tempFeatures$device_memory_read_transactions == 0] <- 
            min(tempFeatures$device_memory_read_transactions)
    ...
    for(iML in c("lm")){
        ...
        for(threshCorr in c(0.5, 0.75)){
            ...
            png(filename = paste("./images/phase3/correlation/heatMap_All_App_GPUs ", "-Thresh=", threshCorr, ".png", sep=""), width = 1600, height = 800)
            heatmap(x = cor(apply(tempData, 2, normalizeLogMax),
                            method = "spearman", use = "complete.obs"),
                    col = col, symm = TRUE)
            dev.off()
            
            png(filename = paste("./images/phase3/correlation/corClustring_All_App_GPUs", "-Thresh=", threshCorr, ".png", sep=""), width = 1600, height = 800)
            ...
            if(length(tempData) > 10){
                ...
                for(numberFeatures in c(5, 10)){
                    ...
                    png(filename = paste("./images/phase3/cluster/All_App_GPUs",
                    ...
                    # png(filename = paste("./images/phase3/scatterPlot/All_App_GPUs", "-Thresh=", threshCorr, "-NParam=", numberFeatures, ".png", sep=""), width = 1600, height = 800)
                    # scatterplotMatrix(Data,cex.labels =  1.5)
                    # dev.off()
                    
                    meanAcc <- array()
                    png(filename = paste("./images/phase3/fitModels/", iML, "_All_App_GPUs", "-Thresh=", threshCorr, "-NParam=", numberFeatures, ".png", sep=""), width = 1600, height = 800)
                    ...
                    png(filename = paste("./images/phase3/features/All_App_GPUs", "-Thresh_", threshCorr, "-NoFeatures_",numberFeatures, ".png", sep=""), width = 1200, height = 2800)
                    ...
                    if(numberFeatures ==  10){
                        write.csv(cbind(names(Data), c(meanAcc, rep(0,4))), file(paste("./results/phase3/", iML, "_All_App_GPUs", "-Thresh_", threshCorr, "-NoFeatures_",numberFeatures,".csv",sep="")))
                    } else{
                        write.csv(cbind(c(names(Data), 0), meanAcc), file(paste("./results/phase3/", iML, "_All_App_GPUs", "-Thresh_", threshCorr, "-NoFeatures_",numberFeatures,".csv",sep="")))                
                    }
                }
            }
        }
        ...
        ggsave(paste("./images/phase3/", iML, "_All_App_GPUs.png",sep=""), Graph, height=10, width=20)
        write.csv(Result, file(paste("./results/phase3/", iML, "-All-App-GPUs.csv",sep="")))
    }
